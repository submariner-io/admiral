...
var (
	...
	_ = Describe("Metrics reporting", testMetricsReporting)
)

func testPoolCreation() {
	When("the CIDR is missing the prefix", func() {
		It("should return an error", func() {
			pool, err := ipam.NewIPPool("169.254.0.0", nil)
	...
	When("the CIDR prefix is /33", func() {
		It("should return an error", func() {
			pool, err := ipam.NewIPPool("169.254.0.0/33", nil)
			Expect(err).To(HaveOccurred())
			Expect(pool).To(BeNil())
		})
	})
	...
	When("the CIDR Prefix is /32", func() {
		It("should return an error", func() {
			pool, err := ipam.NewIPPool("169.254.1.0/32", nil)
			Expect(err).To(HaveOccurred())
			Expect(pool).To(BeNil())
		})
	})
	...
	When("CIDR Prefix is /31", func() {
		It("should return an error", func() {
			pool, err := ipam.NewIPPool("169.254.1.0/31", nil)
			Expect(err).To(HaveOccurred())
			Expect(pool).To(BeNil())
		})
	})
	...
	When("CIDR Prefix is /30", func() {
		It("should create the pool with size 2", func() {
			pool, err := ipam.NewIPPool(cidrWithSize2, nil)
			Expect(err).To(Succeed())
			Expect(pool.Size()).Should(Equal(2))
			Expect(pool.GetCIDR()).To(Equal(cidrWithSize2))
		})
	})

	When("CIDR Prefix is /24", func() {
		It("should create the pool with size 254", func() {
			pool, err := ipam.NewIPPool("169.254.1.0/24", nil)
			Expect(err).To(Succeed())
	...
	When("CIDR Prefix is /16", func() {
		It("should create the pool with size 65534", func() {
			pool, err := ipam.NewIPPool("169.254.1.0/16", nil)
			Expect(err).To(Succeed())
...
func testPoolReserve() {
	...
	Specify("with no IPs should not return an error", func() {
		Expect(t.pool.Reserve()).To(Succeed())
	})
}
...
func testMetricsReporting() {
	t := newTestDriver()

	var metricsCh chan string

	BeforeEach(func() {
		metricsCh = make(chan string, 100)
		t.metrics = ipam.MetricsReporterFuncs{
			RecordAvailabilityFunc: func(cidr string, count int) {
				Expect(cidr).To(Equal(t.cidr))
				metricsCh <- fmt.Sprintf("availabile/%d", count)
			},
			RecordIPsAllocatedFunc: func(cidr string, count int) {
				Expect(cidr).To(Equal(t.cidr))
				metricsCh <- fmt.Sprintf("allocated/%d", count)
			},
			RecordIPDeallocatedFunc: func(cidr string) {
				Expect(cidr).To(Equal(t.cidr))
				metricsCh <- "deallocated"
			},
		}
	})

	When("an IP block is allocated and released", func() {
		It("should report it", func() {
			Expect(metricsCh).To(Receive(Equal("availabile/254")))

			count := 10
			ips := t.allocate(count)
			Expect(metricsCh).To(Receive(Equal(fmt.Sprintf("allocated/%d", count))))

			Expect(t.pool.Release(ips...)).To(Succeed())

			for i := 1; i <= count; i++ {
				Expect(metricsCh).To(Receive(Equal("deallocated")))
			}
		})
	})
}
...
func newTestDriver() *testDriver {
	...
	JustBeforeEach(func() {
		...
		t.pool, err = ipam.NewIPPool(t.cidr, t.metrics)
		Expect(err).To(Succeed())
	})
